...
/**
 * Service for validating clothing items to ensure security and data integrity.
 * Addresses three main security concerns:
 * 1. Ownership verification - ensures users only broadcast clothes they own
 * 2. Item existence check - validates that clothing items exist in database
 * 3. Data integrity - prevents client-side manipulation
 */
public class ClothingValidationService extends DataServiceBase {
    ...
    public Map<Integer, ObjectMap<String, Object>> validateClothingOwnership(
        ...
        if (clothesData == null || clothesData.isEmpty()) {
            ...
            if (hasClothingItems) {
                ...
                logger.warn("User " + user.getLogin() + " has clothing items but sent empty clothing data - possible security violation");
                ...
                logger.debug("Empty clothing data received from user " + user.getLogin() + " - user has no clothing items");
                return new HashMap<>();
            }
        }
        ...
        for (Map.Entry<Integer, ObjectMap<String, Object>> entry : clothesData.entrySet()) {
            ...
            if (ownedItem == null) {
                throw new SecurityException("User attempting to use clothing item they don't own: " + itemId);
            }
            
            // Additional validation: ensure it's actually a clothing item
            if (!StuffTypes.CLOTHES.equals(ownedItem.getType().getType())) {
                throw new SecurityException("User attempting to use non-clothing item as clothing: " + itemId);
            }
     ...
     * @throws SecurityException if validation fails
     */
    public Map<Long, StuffItem> validateItemExistence(List<Long> itemIds, org.hibernate.Session session) {
    ...
    public Map<Integer, ObjectMap<String, Object>> validateClothingData(
        ...
        Map<Integer, ObjectMap<String, Object>> integrityValidated = 
            validateDataIntegrity(clothesData);
    ...
    /**
     * Creates a validation service instance that can be used outside of the service framework.
     * This method should be used when the service is not being called through the normal service framework.
     * 
