...
public class RainCommandService {
     ...
     */
    public static void triggerRainEvent(Session session, String fileName, Integer count, String source) {
     ...
     * @throws IllegalArgumentException if the item is not rainable
     */
    public static void triggerRainEvent(Session session, String fileName, Integer count, String source, Integer serverId, String remoteId) {
        // Get the stuff type
        ...
        Server currentServer = serverDAO.findByUrl(KavalokApplication.getInstance().getCurrentServerPath());
        ...
        if ("admin".equals(source) && serverId != null && remoteId != null && AccessAdmin.class.equals(userAdapter.getAccessType())) {
        ...
        for (String userLogin : connectedUsers) {
            for (int i = 0; i < count; i++) {
                ...
                String rainToken = RainTokenManager.getInstance().generateToken(stuffType.getId().intValue(), uniqueColor);
